...
describe("forum frontend regression checks", () => {
  ...
  it("keeps a writable trailing paragraph after image insertion in editor", () => {
    ...
    expect(source).toContain("if (!previousIsImage || trailingEmptyParagraphCount === 1) {");
    expect(source).toContain("if (trailingEmptyParagraphCount === 0) {");
    expect(source).toContain("transaction = transaction.delete(deleteFrom, doc.content.size).insert(deleteFrom, paragraphNode);");
    ...
    expect(source).toContain("editorInstance.view.dispatch(transaction);");
    expect(source).toContain("const previousBottomGap = editorDom");
    expect(source).toContain("if (previousBottomGap <= 96) {");
    expect(source).toContain("editorDom.scrollTop = editorDom.scrollHeight;");
