...
const createInitDbDomain = (deps) => {
  ...
  const ensureDefaultFollowBookmarkList = async ({ userId, now }) => {
    ...
    const slugConflict = await dbGet(
      "SELECT id FROM manga_bookmark_lists WHERE user_id = ? AND lower(slug) = lower('theo-doi') LIMIT 1",
      [safeUserId]
    );
    const fallbackSlug = slugConflict ? `theo-doi-${timestamp}` : "theo-doi";
    ...
    try {
      inserted = await dbRun(
        `
          ...
          VALUES (?, 'Theo dõi', ?, false, true, NULL, ?, ?)
          RETURNING id
        `,
        [safeUserId, fallbackSlug, timestamp, timestamp]
      );
    } catch (_err) {
      const recovery = await dbGet(
        `
          ...
          FROM manga_bookmark_lists
          WHERE user_id = ?
            AND (is_default_follow = true OR lower(name) = lower('Theo dõi'))
          ORDER BY is_default_follow DESC, updated_at DESC, id DESC
          LIMIT 1
        `,
        [safeUserId]
      );
      if (recovery && recovery.id != null) {
        await dbRun(
          "UPDATE manga_bookmark_lists SET is_default_follow = CASE WHEN id = ? THEN true ELSE false END WHERE user_id = ?",
          [recovery.id, safeUserId]
        );
      }
    }

    const insertedId = inserted && Array.isArray(inserted.rows) && inserted.rows[0]
      ? Number(inserted.rows[0].id)
      : NaN;
    if (Number.isFinite(insertedId) && insertedId > 0) {
      return Math.floor(insertedId);
    }
    ...
    return fallback && fallback.id != null ? Number(fallback.id) || 0 : 0;
  };

  const migrateBookmarkListDataV1 = async () => {
    const now = Date.now();
    ...
    await dbRun(
      `
        ...
        SET
          slug = CASE
            WHEN slug IS NULL OR TRIM(slug) = '' THEN 'list-' || id::text
            ELSE slug
          END,
          updated_at = COALESCE(updated_at, created_at, ?)
        WHERE slug IS NULL OR TRIM(slug) = '' OR updated_at IS NULL
      `,
      [now]
    );
    ...
    for (const row of users) {
      ...
      const followListId = await ensureDefaultFollowBookmarkList({ userId, now });
      if (!followListId) continue;

      await dbRun(
        `
          INSERT INTO manga_bookmark_list_items (list_id, manga_id, created_at, updated_at)
          SELECT ?, mb.manga_id, COALESCE(mb.created_at, ?), COALESCE(mb.updated_at, mb.created_at, ?)
          FROM manga_bookmarks mb
          WHERE mb.user_id = ?
          ON CONFLICT (list_id, manga_id)
          DO UPDATE SET updated_at = EXCLUDED.updated_at
        `,
        [followListId, now, now, userId]
      );
    }
  };
  ...
  const ensurePushSubscriptionsSchema = async () => {
    await dbRun(
      `
    ...
    await dbRun("ALTER TABLE push_subscriptions ADD COLUMN IF NOT EXISTS user_id TEXT");
    await dbRun("ALTER TABLE push_subscriptions ADD COLUMN IF NOT EXISTS endpoint TEXT");
    await dbRun("ALTER TABLE push_subscriptions ADD COLUMN IF NOT EXISTS p256dh TEXT");
    ...
    await dbRun("ALTER TABLE push_subscriptions ADD COLUMN IF NOT EXISTS last_error_at BIGINT");
    await dbRun("ALTER TABLE push_subscriptions ADD COLUMN IF NOT EXISTS last_error_code TEXT");
    await dbRun(
...
const initDb = async () => {
  ...
  await dbRun("ALTER TABLE manga ADD COLUMN IF NOT EXISTS id INTEGER GENERATED BY DEFAULT AS IDENTITY");
  await dbRun("CREATE UNIQUE INDEX IF NOT EXISTS idx_manga_id ON manga(id)");
  ...
  await dbRun("ALTER TABLE chapter_reports ADD COLUMN IF NOT EXISTS reporter_user_id TEXT");
  ...
  await dbRun("ALTER TABLE forum_post_image_drafts ADD COLUMN IF NOT EXISTS user_id TEXT");
  await dbRun("ALTER TABLE forum_post_image_drafts ADD COLUMN IF NOT EXISTS images_json TEXT NOT NULL DEFAULT '[]'");
  await dbRun("ALTER TABLE forum_post_image_drafts ADD COLUMN IF NOT EXISTS created_at BIGINT");
  await dbRun("ALTER TABLE forum_post_image_drafts ADD COLUMN IF NOT EXISTS updated_at BIGINT");
  await dbRun("ALTER TABLE forum_post_image_drafts DROP COLUMN IF EXISTS manga_slug");
  await dbRun("CREATE INDEX IF NOT EXISTS idx_forum_post_image_drafts_updated_at ON forum_post_image_drafts(updated_at)");
  ...
  await dbRun("ALTER TABLE forum_posts ADD COLUMN IF NOT EXISTS author_name TEXT");
  ...
  await dbRun(
    "UPDATE forum_posts SET author_name = COALESCE(NULLIF(TRIM(author_name), ''), NULLIF(TRIM(author), ''), 'Ẩn danh') WHERE author_name IS NULL OR TRIM(author_name) = ''"
  );
  ...
  await dbRun(
    `
      CREATE TABLE IF NOT EXISTS manga_translation_teams (
        manga_id INTEGER NOT NULL REFERENCES manga(id) ON DELETE CASCADE,
        team_id INTEGER NOT NULL REFERENCES translation_teams(id) ON DELETE CASCADE,
        PRIMARY KEY (manga_id, team_id)
      )
    `
  );
  ...
  await dbRun(
    `
    CREATE TABLE IF NOT EXISTS manga_bookmark_lists (
      id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
      user_id TEXT NOT NULL REFERENCES users(id) ON DELETE CASCADE,
      name TEXT NOT NULL,
      slug TEXT NOT NULL,
  ...
  await dbRun("ALTER TABLE manga_bookmark_lists ADD COLUMN IF NOT EXISTS user_id TEXT");
  await dbRun("ALTER TABLE manga_bookmark_lists ADD COLUMN IF NOT EXISTS name TEXT");
  await dbRun("ALTER TABLE manga_bookmark_lists ADD COLUMN IF NOT EXISTS slug TEXT");
  await dbRun("ALTER TABLE manga_bookmark_lists ADD COLUMN IF NOT EXISTS is_public BOOLEAN NOT NULL DEFAULT false");
  await dbRun("ALTER TABLE manga_bookmark_lists ADD COLUMN IF NOT EXISTS is_default_follow BOOLEAN NOT NULL DEFAULT false");
  await dbRun("ALTER TABLE manga_bookmark_lists ADD COLUMN IF NOT EXISTS share_token TEXT");
  await dbRun("ALTER TABLE manga_bookmark_lists ADD COLUMN IF NOT EXISTS created_at BIGINT");
  await dbRun("ALTER TABLE manga_bookmark_lists ADD COLUMN IF NOT EXISTS updated_at BIGINT");
  await dbRun("DELETE FROM manga_bookmark_lists WHERE user_id IS NULL OR TRIM(user_id) = ''");
  await dbRun("UPDATE manga_bookmark_lists SET name = 'Danh sách' WHERE name IS NULL OR TRIM(name) = ''");
  await dbRun("UPDATE manga_bookmark_lists SET slug = 'list-' || id::text WHERE slug IS NULL OR TRIM(slug) = ''");
  await dbRun("UPDATE manga_bookmark_lists SET created_at = ? WHERE created_at IS NULL", [Date.now()]);
  await dbRun("UPDATE manga_bookmark_lists SET updated_at = COALESCE(updated_at, created_at, ?) WHERE updated_at IS NULL", [Date.now()]);
  await dbRun("ALTER TABLE manga_bookmark_lists ALTER COLUMN user_id SET NOT NULL");
  await dbRun("ALTER TABLE manga_bookmark_lists ALTER COLUMN name SET NOT NULL");
  await dbRun("ALTER TABLE manga_bookmark_lists ALTER COLUMN slug SET NOT NULL");
  await dbRun("ALTER TABLE manga_bookmark_lists ALTER COLUMN is_public SET DEFAULT false");
  await dbRun("ALTER TABLE manga_bookmark_lists ALTER COLUMN is_public SET NOT NULL");
  await dbRun("ALTER TABLE manga_bookmark_lists ALTER COLUMN is_default_follow SET DEFAULT false");
  await dbRun("ALTER TABLE manga_bookmark_lists ALTER COLUMN is_default_follow SET NOT NULL");
  await dbRun("ALTER TABLE manga_bookmark_lists ALTER COLUMN created_at SET NOT NULL");
  await dbRun("ALTER TABLE manga_bookmark_lists ALTER COLUMN updated_at SET NOT NULL");
  await dbRun("CREATE INDEX IF NOT EXISTS idx_manga_bookmark_lists_user_updated ON manga_bookmark_lists(user_id, updated_at DESC, id DESC)");
  await dbRun("CREATE UNIQUE INDEX IF NOT EXISTS idx_manga_bookmark_lists_user_slug_lower ON manga_bookmark_lists(user_id, lower(slug))");
  await dbRun("CREATE UNIQUE INDEX IF NOT EXISTS idx_manga_bookmark_lists_user_name_lower ON manga_bookmark_lists(user_id, lower(name))");
  await dbRun("CREATE UNIQUE INDEX IF NOT EXISTS idx_manga_bookmark_lists_share_token ON manga_bookmark_lists(share_token) WHERE share_token IS NOT NULL");
  await dbRun("CREATE UNIQUE INDEX IF NOT EXISTS idx_manga_bookmark_lists_default_follow ON manga_bookmark_lists(user_id) WHERE is_default_follow = true");

  await dbRun(
    `
    CREATE TABLE IF NOT EXISTS manga_bookmark_list_items (
      list_id INTEGER NOT NULL REFERENCES manga_bookmark_lists(id) ON DELETE CASCADE,
      manga_id INTEGER NOT NULL REFERENCES manga(id) ON DELETE CASCADE,
      created_at BIGINT NOT NULL,
      updated_at BIGINT NOT NULL,
  ...
  await dbRun("ALTER TABLE manga_bookmark_list_items ADD COLUMN IF NOT EXISTS list_id INTEGER");
  await dbRun("ALTER TABLE manga_bookmark_list_items ADD COLUMN IF NOT EXISTS manga_id INTEGER");
  await dbRun("ALTER TABLE manga_bookmark_list_items ADD COLUMN IF NOT EXISTS created_at BIGINT");
  await dbRun("ALTER TABLE manga_bookmark_list_items ADD COLUMN IF NOT EXISTS updated_at BIGINT");
  await dbRun("DELETE FROM manga_bookmark_list_items WHERE list_id IS NULL OR manga_id IS NULL");
  await dbRun("UPDATE manga_bookmark_list_items SET created_at = ? WHERE created_at IS NULL", [Date.now()]);
  await dbRun("UPDATE manga_bookmark_list_items SET updated_at = COALESCE(updated_at, created_at, ?) WHERE updated_at IS NULL", [Date.now()]);
  await dbRun("ALTER TABLE manga_bookmark_list_items ALTER COLUMN list_id SET NOT NULL");
  await dbRun("ALTER TABLE manga_bookmark_list_items ALTER COLUMN manga_id SET NOT NULL");
  await dbRun("ALTER TABLE manga_bookmark_list_items ALTER COLUMN created_at SET NOT NULL");
  await dbRun("ALTER TABLE manga_bookmark_list_items ALTER COLUMN updated_at SET NOT NULL");
  await dbRun("CREATE INDEX IF NOT EXISTS idx_manga_bookmark_list_items_list_updated ON manga_bookmark_list_items(list_id, updated_at DESC, manga_id DESC)");
  await dbRun("CREATE INDEX IF NOT EXISTS idx_manga_bookmark_list_items_manga_id ON manga_bookmark_list_items(manga_id)");
